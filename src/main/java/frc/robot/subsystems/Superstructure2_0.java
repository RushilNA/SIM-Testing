 ...
 * Superstructure2_0
 *
 * Coordinator for Elevator + Arm + Intake.
 * - Minimal transitions (no auto-home).
 * - Safety nets ONLY in STOW: before moving the arm to stow, auto-raise elevator to a clearance height.
 * - Parallel Elevator+Arm when safe (outside STOW).
...
public class Superstructure2_0 extends SubsystemBase {
  ...
  private static final class Goal {
    ...
    Goal(ElevatorMode e, ArmMode a, IntakeMode i) {
      this.elevator = e; this.arm = a; this.intake = i;
    }
  }
  ...
  private static final class SafetyRules {
    static boolean requireStow(State from, State to) {
      ...
      return (to == State.L4 || to == State.BARGE)
          && (from == State.INTAKE || from == State.L1);
    }
  ...
  static {
    ...
    GOALS.put(State.INTAKE_SOURCE,  new Goal(ElevatorMode.INTAKE,    ArmMode.INTAKE, IntakeMode.INTAKE /* SOURCE later */));
  ...
  public State getRequestedState() { return requestedState; }
  public State getCurrentState()   { return currentState;  }
  public boolean isBusy()          { return phase != Phase.IDLE && phase != Phase.HOLD; }
    ...
    switch (phase) {
      case STOW: {
        ...
        if (arm.isAtTarget() && intake.isAtTarget()) {
          ...
          phase = Phase.MOVE_ELEVATOR_TO_FINAL; // ensure elevator goes to the requested final, not just the clearance
          timer.reset();
        }
        break;
      }
